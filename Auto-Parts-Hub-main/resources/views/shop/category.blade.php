...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="relative h-[400px] overflow-hidden">
        <img src="{{ asset($category->image_path ?? 'images/categories/default-category.jpg') }}" 
             alt="{{ $category->name }}" 
                    ...
                    <nav class="flex mb-6 text-gray-300" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            <li class="inline-flex items-center">
                                <a href="{{ route('shop.index') }}" class="hover:text-white transition">Shop</a>
                            </li>
    ...
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="text-center mb-12">
            <h2 class="text-3xl font-bold text-gray-900">Browse {{ $category->name }} Categories</h2>
            ...
            <a href="{{ route('shop.category', ['slug' => $subcategory->slug]) }}" 
               class="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300">
                <div class="relative h-56">
                    <img src="{{ asset($subcategory->image_path ?? 'images/categories/default-subcategory.jpg') }}" 
                         alt="{{ $subcategory->name }}" 
                         class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                        <div class="absolute bottom-0 left-0 right-0 p-6">
                            <h3 class="text-2xl font-bold text-white mb-2">{{ $subcategory->name }}</h3>
                            @if($subcategory->products_count)
                                <span class="inline-flex items-center text-white/90 text-sm">
                                    ...
                                    {{ $subcategory->products_count }} Products Available
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                <div class="p-6">
                    <p class="text-gray-600 mb-4 line-clamp-2">{{ $subcategory->description }}</p>
